class Group::AddUserToConversationService
  ...
  def call
    ...
    if new_user.group_conversations << group_conversation
      create_info_message(new_user, added_by, group_conversation) # Pass group_conversation
    end
  end

  private

  def create_info_message(new_user, added_by, group_conversation)
    message = Group::Message.new(
      user_id: added_by.id,
      content: "#{new_user.name} added by #{added_by.name}",
      added_new_users: [new_user.id],
      conversation_id: group_conversation.id # Use conversation_id
    )
    if message.save
